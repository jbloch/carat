...
Carat is a lightweight, automated GUI utility designed to ingest Dolby Atmos music releases from practically any format and integrate them directly into your digital music library in a single click. 
...
As the landscape evolves, we may extend Carat's capabilities to support more input and output formats, but it will remain laser-focused on one task: quickly ingesting Atoms albums into your digital music library.
...
* **Run Headless:** Execute Carat directly from the command line or import it programmatically into custom Python scripts for massive batch-processing workflows.
* **Standalone Cover Art:** The `get_cover_art` module can be used entirely in isolation to scrape top-tier artwork from MusicBrainz/CAA and Apple/iTunes to upgrade your existing digital library.
